...
def run_ga_ql(env, runs, episodes, ql_params, ga_params, delta):
    ...
    for r in range(runs):
        ...
        for tau in range(episodes):
            ...
            print(
                f'Run {r+1}/{runs}  -  Episode {tau+1}/{episodes}  -  Episode Reward: {rewards.mean()}', end='\r')
    print('')
...
def parse_args():
    ...
    parser.add_argument(
        '--s', help='Max number of steps taken by an agent.', default=2, type=int)
    parser.add_argument(
        '--k', help='Max number of shortest paths from an origin to a destination node.', default=2, type=int)
    # GA Params
    parser.add_argument(
        '--delta', help='Frequency of interaction GA -> QL.', default=10, type=int)
    parser.add_argument('--population', '-p',
                        help='Size of population.', default=100, type=int)
    parser.add_argument('--crossover_rate', '-c',
                        help='Rate of crossover.', default=0.5, type=float)
    parser.add_argument('--mutation_rate', '-m',
                        help='Rate of mutation.', default=0.5, type=float)
    parser.add_argument('--eletism', '-e', help='Keep the most fit individual in the population. Parameter does not take any arguments.',
                        default=False, action='store_true')
    # QLearn Params
    parser.add_argument(
        '--epsilon', '-eps', help='Starting epsilon for QLearning.', default=.05, type=float)
    parser.add_argument(
        '--decay', help='Epsilon decay rate.', default=1, type=float)
    parser.add_argument(
        '--alpha', help='Learning rate alpha value of QLearning.', default=0.5, type=float)
    # Simulation Params
    parser.add_argument(
        '--runs', help='Number of runs for GA-QL.', default=1, type=int)
    parser.add_argument(
        '--episodes', help='Number of episodes to run.', default=500, type=int)
    parser.add_argument(
        '--outdir', help='Output dir for the plot.', default='./figs/')
